...
png(file.path(soypath, 'figures', 'FIG03_network_stats_time.png'), width=400, height=400)
grid.arrange(conn.grob, dens.grob, ncol=1)
dev.off()  
...
Nlength.ties.plot = ggplot(Nlength.ties.df, aes(x=number, y=length)) + 
  geom_point() + theme_bw() + ggtitle("B") + 
  labs(x='Degree Centrality by Country', y='Connected Duration by Country') + 
  theme(text = element_text(size=16)) 

#### VALUES FOR AXES ON 4C ARE SWAPPED RELATIVE TO PREVIOUS VERSIONS.
# THe only thing that has changed is how we import nd2, which now makes
# more sense than it was previously. But it is unexpected that the axes
# would be swapped. So now sending events is 0-150, but receiving is 0-300,
# but before it was the other way around.

# Figure 4c
## ratio of export events to import events?
duration.df = data.frame(send=tiedDuration(nd2, mode='counts',neighborhood = 'out'), 
                         receive=tiedDuration(nd2, mode='counts',neighborhood = 'in'))
duration.plot = ggplot(duration.df, aes(x=send, y=receive)) + 
  geom_point() + theme_bw() + ggtitle("C") +
...
grob.list = list(edgedur.grob,Nlength.ties.grob,duration.grob)
grob.list[[1]]$widths
grob.list[[2]]$widths
grob.list[[3]]$widths

Fig4maxWidth = grid::unit.pmax(grob.list[[1]]$widths[2:5], grob.list[[2]]$widths[2:5], grob.list[[3]]$widths[2:5])
grob.list[[1]]$widths[2:5] = grob.list[[2]]$widths[2:5] = grob.list[[3]]$widths[2:5] = as.list(Fig4maxWidth)
grid.arrange(grobs=grob.list, ncol=1)

  png(file.path(soypath, 'figures', 'FIG04_network_stats.png'), width=600, height=600)
...
btw.lm4 = lm(US.btw ~ Ar_Ch.prop + Br_Ch.prop + Ur_Ch.prop + Pa_Ch.prop, spillover.scale) # S Am exports to China
btw.lm5 = lm(US.btw ~ SouthAm_Ch.prop, spillover.scale) # Sum of S Am exports to China
btw.lm6 = lm(US.btw ~ Ar_Ch.prop + Br_Ch.prop + Ur_Ch.prop + Pa_Ch.prop + Ca_Ch.prop + Ru_Ch.prop + Au_Ch.prop + SA_Ch.prop + In_Ch.prop + Et_Ch.prop, spillover.scale) # top 10 exporters other than USA
btw.lm7 = lm(US.btw ~ TopTen_Ch.prop, spillover.scale) # top 10 exporters other than USA
btw.lm8 = lm(US.btw ~ all.but.us.br.ar_ch.prop, spillover.scale) # sum of all but Brazil, Argentina, and USA
...
btw.lm9 = lm(US.btw ~ Br_Ch.prop + Ar_Ch.prop + SA_Ch.prop, spillover.scale) # Brazil, Argentina, South Africa
btw.lm10 = lm(US.btw ~ TopTen_Ch.prop + SouthAm_Ch.prop + all.but.us.br.ar_ch.prop + Ar_Ch.prop + Br_Ch.prop + Ur_Ch.prop + Pa_Ch.prop + Ca_Ch.prop + Ru_Ch.prop + Au_Ch.prop + SA_Ch.prop + In_Ch.prop + Et_Ch.prop, spillover.scale) # full model


btw.mods = list(btw.lm0,btw.lm1,btw.lm2,btw.lm3,btw.lm4,btw.lm5,btw.lm6,btw.lm7,btw.lm8,btw.lm9,btw.lm10)
btw.modnames = names(btw.mods) = paste0("btw.lm",(1:length(btw.mods))-1)
sink(file.path(soypath, "figures", "Spillovers_models_betweenness.txt"))
lapply(btw.mods,summary); sink()


# model selection
library(MuMIn)
btw.modelselect.df = as.data.frame(model.sel(btw.mods))
(btw.modsel.df = btw.modelselect.df[,c('df','AICc','delta','weight')])
...
deg.lm4 = lm(US.deg ~ Ar_Ch.prop + Br_Ch.prop + Ur_Ch.prop + Pa_Ch.prop, spillover.scale) # S Am exports to China
deg.lm5 = lm(US.deg ~ SouthAm_Ch.prop, spillover.scale) # Sum of S Am exports to China
deg.lm6 = lm(US.deg ~ Ar_Ch.prop + Br_Ch.prop + Ur_Ch.prop + Pa_Ch.prop + Ca_Ch.prop + Ru_Ch.prop + Au_Ch.prop + SA_Ch.prop + In_Ch.prop + Et_Ch.prop, spillover.scale) # top 10 exporters other than USA
deg.lm7 = lm(US.deg ~ TopTen_Ch.prop, spillover.scale) # top 10 exporters other than USA
deg.lm8 = lm(US.deg ~ all.but.us.br.ar_ch.prop, spillover.scale) # sum of all but Brazil, Argentina, and USA
...
deg.lm9 = lm(US.deg ~ Br_Ch.prop + Ar_Ch.prop + SA_Ch.prop, spillover.scale) # Brazil, Argentina, South Africa
deg.lm10 = lm(US.deg ~ TopTen_Ch.prop + SouthAm_Ch.prop + all.but.us.br.ar_ch.prop + Ar_Ch.prop + Br_Ch.prop + Ar_Ch.prop:Br_Ch.prop + Ur_Ch.prop + Pa_Ch.prop + Ca_Ch.prop + Ru_Ch.prop + Au_Ch.prop + SA_Ch.prop + In_Ch.prop + Et_Ch.prop, spillover.scale) # full model
...
(deg.modsel.df = deg.modelselect.df[,c('df','AICc','delta','weight')])
