...
library(countrycode)
s <- soy.cleaned$Reporter.Countries
...
names(ch.melt.df)[9] = "proportion"

###########################################
# Temporal graphs by proportion : FIGURE 7
br.melt.df$receiver = factor(br.melt.df$receiver, levels=c("China","Netherlands","Spain","Germany","Thailand"))
br.prop.plot = ggplot(data=br.melt.df, aes(x=t1, y=proportion, group=receiver, color=receiver)) +
  geom_line() + ggtitle("Top 5 receivers of soy sent by Brazil") +
  labs(y="proportion of total \nBrazil soy exports", x="year") +
  scale_color_manual(values=c("#d95f02", "#66a61e", "#e6ab02", "#a6761d", "#666666")) +
  theme_bw()

us.melt.df$receiver = factor(us.melt.df$receiver, levels=c("China","Japan","Mexico","Netherlands","Indonesia"))
us.prop.plot = ggplot(data=us.melt.df, aes(x=t1, y=proportion, group=receiver, color=receiver)) +
  geom_line() + ggtitle("Top 5 receivers of soy sent by the US") +
  labs(y="proportion of total \nUS soy exports", x="year") +
  scale_color_manual(values=c("#d95f02", "#66a61e", "#e6ab02", "#a6761d", "#666666")) +
  theme_bw()

ch.melt.df$sender = factor(ch.melt.df$sender, levels=c("United States","Brazil","Argentina","Canada","Uruguay"))
ch.prop.plot = ggplot(data=ch.melt.df, aes(x=t1, y=proportion, group=sender, color=sender)) +
  geom_line() + ggtitle("Top 5 senders of soy received by China") +
  labs(y="proportion of total \nChina soy imports", x="year") +
  scale_color_manual(values=c("#1b9e77", "#7570b3", "#e6ab02", "#a6761d", "#666666")) +
  theme_bw()
...
Nlength.ties.plot = ggplot(Nlength.ties.df, aes(x=number, y=length)) + geom_point() + 
  theme_bw() + labs(x='Degree Centrality', y='Connected Duration') 
