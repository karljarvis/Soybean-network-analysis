...
links.df <- data.frame(
  ...
  qtyM = soy.cleaned.dollars$sum * 1000) # $1000 units, convert to $1M

## change links dataset to have numbers instead of countries
country=as.character(sort(unique(links.df$receiver)))
senderindex = cbind(sender=country, sendindex=1:length(country))
receiverindex = cbind(receiver=country, receiveindex=1:length(country))
links.m1 = merge(links.mat, senderindex, by="sender")
links.m2 = merge(links.m1, receiverindex, by="receiver")
links.m = as.matrix(data.frame(links.m2$t1, links.m2$t2, links.m2$receiveindex, 
                                          links.m2$sendindex, links.m2$qtyM))
links.m = apply(links.m, 2, as.numeric)
colnames(links.m) = names(links.df)
apply(links.m, 2, class)
...
### Create the network ----
nd2 <- networkDynamic(edge.spells = links.m, 
                      create.TEAs = TRUE, 
...
#   ch.df = ch.links[!ch.links$sender == "China",c("t1","qtyM","sender")]
#   ch.sum.year = ddply(ch.df, .(t1), summarize, total=sum(qtyM))
#   ch.df = merge(ch.df, ch.sum.year)
#   ch.df$proportion = 1-(ch.df$total - ch.df$qtyM)/ch.df$total
...
# rank by total soy exported to China
  ch.sums.df = ddply(ch.links, .(sender), summarize, total=sum(qtyM))
