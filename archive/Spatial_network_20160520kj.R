...
soypath = file.path("~/Dropbox/NASA-MSU_2014/Soybean_network_analysis")
...
soy <- read.csv(file.path(soypath,"data/FAOSTAT_matrix_soybeans.csv"), header = T, stringsAsFactors = F)
...
# group countries 
# could add on other country dataset with GDP, population, etc.
# but would need easier way of representing country names
# could add things like deforestation in. 
# meat production and consumption links to soy, because most soy is used for feed
# global drivers of deforestation
# how to represent directionality of flow? 
# could just focus on single pairwise relationship, and plot change over time
...
# Are countries that are tightly linked - are they more linked than we'd expect
