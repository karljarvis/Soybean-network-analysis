...
# Temporal graphs
br.plot = ggplot(data=br.top5.df, aes(x=t1, y=qty, group=receiver, color=receiver)) +
  geom_line() + ggtitle("Top 5 receivers of Brazil soy exports") +
  scale_color_brewer(palette="Set1") +
  theme_bw()

us.plot = ggplot(data=us.top5.df, aes(x=t1, y=qty, group=receiver, color=receiver)) +
  geom_line() + ggtitle("Top 5 receivers of US soy exports") +
  scale_color_brewer(palette="Set1") +
  theme_bw()

ch.plot = ggplot(data=ch.top5.df, aes(x=t1, y=qty, group=sender, color=sender)) +
  geom_line() + ggtitle("Top 5 producers of China soy imports") +
  scale_color_brewer(palette="Set1") +
  theme_bw()

library(gridExtra)
png(file.path(soypath, 'figures', 'temporal_brazil_us_china.png'))
grid.arrange(br.plot, us.plot, ch.plot)
dev.off()


# proportion of total edge weight per year
br.sum.year = ddply(br.links, .(t1), summarize, total=sum(qty))
br.cast = dcast(br.top5.df, t1 ~ receiver, value.var="qty")
br.prop = apply(br.cast[,2:6], 2,function(x){1-(br.sum.year$total-x)/br.sum.year$total})
br.melt = na.omit(melt(br.prop))
br.melt.df = data.frame(br.top5.df, br.melt$value)
names(br.melt.df)[8] = "proportion"

us.sum.year = ddply(us.links, .(t1), summarize, total=sum(qty))
us.cast = dcast(us.top5.df, t1 ~ receiver, value.var="qty")
us.prop = apply(us.cast[,2:6], 2,function(x){1-(us.sum.year$total-x)/us.sum.year$total})
us.melt = na.omit(melt(us.prop))
us.melt.df = data.frame(us.top5.df, us.melt$value)
names(us.melt.df)[8] = "proportion"

ch.sum.year = ddply(ch.links, .(t1), summarize, total=sum(qty))
ch.cast = dcast(ch.top5.df, t1 ~ sender, value.var="qty")
ch.prop = apply(ch.cast[,2:6], 2,function(x){1-(ch.sum.year$total-x)/ch.sum.year$total})
ch.melt = na.omit(melt(ch.prop))
ch.melt.df = data.frame(ch.top5.df, ch.melt$value)
names(ch.melt.df)[8] = "proportion"


# Temporal graphs by proportion
br.prop.plot = ggplot(data=br.melt.df, aes(x=t1, y=proportion, group=receiver, color=receiver)) +
geom_line() + ggtitle("Top 5 receivers of Brazil soy exports") +
scale_color_brewer(palette="Set1") +
theme_bw()

us.prop.plot = ggplot(data=us.melt.df, aes(x=t1, y=proportion, group=receiver, color=receiver)) +
  geom_line() + ggtitle("Top 5 receivers of US soy exports") +
  scale_color_brewer(palette="Set1") +
  theme_bw()

ch.prop.plot = ggplot(data=ch.melt.df, aes(x=t1, y=proportion, group=sender, color=sender)) +
  geom_line() + ggtitle("Top 5 senders of China soy imports") +
  scale_color_brewer(palette="Set1") +
  theme_bw()

library(gridExtra)
png(file.path(soypath, 'figures', 'prop_brazil_us_china.png'))
grid.arrange(br.prop.plot, us.prop.plot, ch.prop.plot)
dev.off()  
