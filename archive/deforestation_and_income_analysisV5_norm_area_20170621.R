...
# Calculate the difference  in exports and imports year to year by country
#library(quantmod)
...
hansen <- read.csv("E:/telecoupling/data/Austin_et_al_2017_data/patch_dynamics_pantropics_reformat_djs_20170516.csv", header = T, stringsAsFactors = F)
...
forest2000 <- read.csv("E:/telecoupling/data/GFW_data/tree_cover_reformat_djs_20170607.csv", header = T, stringsAsFactors = F)
...
gdp <- read.csv("E:/telecoupling/data/GDP_World_Bank_data/API_3_DS2_en_csv_v2_reformat_djs_20170515.csv", header = T, stringsAsFactors = F)
...
income <- read.csv("E:/telecoupling/data/Income_class_World_Bank_data/Income_classes_reformatted_djs_20170515.csv", header = T, stringsAsFactors = F)
...
#write.csv(pantropics, "D:/telecoupling/pantropical_countries_list.csv")
...
test1 <- socioeco%>%
  ...
  mutate(log.sum.exportMUSDcor = log10(sum.exportMUSD/cover.2000.gt50.ha),
         log.sum.importMUSDcor = log10(sum.importMUSD/cover.2000.gt50.ha),
         log.sum.loss.hacor = log10(sum.loss.ha/cover.2000.gt50.ha)
         )

#test1 <- test1[!is.na(test1$income00),] # remove countries which have no starting income group in 2000

## Countries to highlight:
## L: Indonesia, Cameroon
## LM: Dominican Republic, Paraguay
## UM: Brazil, SAfrica
highlight.countries <- c("Indonesia", "Brazil", "Cameroon", "Jamaica", "Paraguay", "South Africa")
...
## Save Figure
#ggsave("D:/telecoupling/figures/CumExports_predict_CumForestLoss_forest_area_correctedV2.png", plot = p1, width = 7.5, height = 5, units = "cm", dpi = 300)
png(file.path('E:/telecoupling/figures', 'FIG05_CumExports_predict_CumForestLoss_forest_area_correctedV3.png'), width=400, height=400)
...
## Save Figure
#ggsave("D:/telecoupling/figures/CumGNIMUSD_predict_CumImportsMUSD_forest_area_correctedV2.png", plot = p2, width = 7.5, height = 5, units = "cm", dpi = 300)
...
## Save Figure
#ggsave("D:/telecoupling/figures/CumGNIMUSD_predict_CumExportsMUSD_forest_area_correctedV2.png", plot = p3, width = 7.5, height = 5, units = "cm", dpi = 300)
