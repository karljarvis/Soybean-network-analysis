...
Nlength.ties.plot = ggplot(Nlength.ties.df, aes(x=number, y=length)) + geom_point() + 
  theme_bw() + labs(x='Degree Centrality', y='Connected Duration (years)') + ggtitle("B") +
  geom_point(colour="black") +
  geom_point(data=Nlength.ties.df[10:13, ], aes(x=a, y=b), colour="red", size=5)
...
for(i in 1:(ncol(s.count)-1))
{
  sp.list[[i]] = lm(s.prop[,i] ~ time, data=s.prop)
}
...
{
  rp.list[[i]] = lm(r.prop[,i] ~ time, data=r.prop)
}
