...
### DS edits: 
# Hi Karl,
# OK I *think* I figured out what was going on--see "temporal_analysis_20180118_DS_edit" on the Drive.  
# I think the network dynamic call was adding countries because it was pulling from "links2" and not "links.df". 
# Also, for the error about not using character names for countries, I think this actually had to do with the way networkDynamic reads in the data (it expects a certain order and to use the code1 and code2 numeric fields to represent the nodes (those should be the fixed ids)). If there are extra columns beyond the ones networkDynamic expects, these need to be named or that throws this same character error. I added named columns for the nd2 dataset (L201), but not the subsequent subnetworks for Brazil, China, US. 
# I added my inelegant code to eliminate self-trades.
# Stopped at L203 in that R code. 
# I hope this gets you off and running!!! Let me know if another issue comes up!  

#Still no luck with it. It is still pulling in country names as factors and changing them to numeric vectors in alphabetical order. Then when you import names, they are in another order.
# BUT... what if I just reorder the 'nodes' object to be aphabetical. Will that work???
...
## Get ids for China, Brazil, US 
head(links.df[links.df$sender == "China", ])# 23 = China
head(links.df[links.df$sender == "Brazil", ])# 15 = Brazil
head(links.df[links.df$sender == "United States", ])# 121 = United States
...
nd2 <- networkDynamic(edge.spells = links.df, create.TEAs = TRUE, edge.TEA.names = c('weight', 'sender', 'receiver', 'weightM'))
nd2

nd2%v%'receiver'
network::get.vertex.attribute(nd2, "sender")

## DS STOPPED HERE 1/22/2018
