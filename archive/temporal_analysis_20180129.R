...
nodes.df = data.frame(countries=nodes, codes=node.num) # df containing country names and a number code for each

## Add coordinate attributes for each node (country centroids)
# nodes.df <- merge(nodes.df, coords, by.x = "iso2", by.y = "ISO3136")
# str(nodes.df) # 217 countries. Some lost due to mismatched names.
# nodes.df$id <- nodes.df$iso2
# nodes.df$latitude <- nodes.df$LAT
# nodes.df$longitude <- nodes.df$LONG
...
links <- data.frame(
  ...
  weight = soy.cleaned.dollars$sum,
...
# problem: can't just bind these and get somewhat random numbers instead of country names

#### FIX links.df - code2 is incorrect (multiple codes for single country, has NA)
#### (use some kind of join function from tidyverse)
# give countries official fao codes
links.r = merge(links, nodes.df, by.x="receiver", by.y="nodes")
links.df = merge(links.r, nodes.df, by.x="sender", by.y="nodes")
names(links.df)[6:7] = c("r.code","s.code")

# links.df = data.frame(
#   t1 = links.df$time,
#   t2 = links.df$time2,
#   s.code = links.df$s.code,
#   r.code = links.df$r.code,
#   weight = links.df$weight,
#   sender = links.df$sender,
#   receiver = links.df$receiver,
#   weightM = links.df$weight*1000
# )

links2 <- cbind(links.df$time, links.df$time2, links.df$s.code, links.df$r.code, links.df$weights)
links.df = data.frame(links2, links$sender, links$receiver)
names(links.df) = c('t1','t2','code1','code2','weight','sender','receiver')


## Remove links where the sender = receiver 
str(links.df) #16963 observations
links.df$sender <- as.character(links.df$sender)
links.df$receiver <- as.character(links.df$receiver)
links.check <- links.df %>% filter(sender != receiver)
links.df <- links.check # overwrite links dataset with corrected version
str(links.df) # 16919 obs when we exclude self trading

## Calculate weights in 1000s USD
# links.df$qtyM = links.df$weights/1000
...
nd2 <- networkDynamic(edge.spells = links.df, create.TEAs = TRUE, edge.TEA.names = c('weight','sender','receiver'))
nd2
...
# us.nd = networkDynamic(edge.spells = us.links[,-8], create.TEAs = TRUE, edge.TEA.names = c('weight'))

#### KJ 1/29/18: trying to get networkDynamic to properly import. Will not import names,
#### so we don't have any way to identify nodes in the nd2 object. 
...
legend('bottomright', legend=c('USA','Brazil','China'), col=c('blue','green','red'), lty=1, bty='n')
dev.off()
...
# show all senders to China
  ch.df = ch.links[!ch.links$sender == "China",c("t1","qty","sender")]
  ch.sum.year = ddply(ch.df, .(t1), summarize, total=sum(qty))
  ch.df = merge(ch.df, ch.sum.year)
  ch.df$proportion = 1-(ch.df$total - ch.df$qty)/ch.df$total
...
### KJ 1/29/2018: regression with US betweenness as response variable
  


USbtw <- sapply(soyNets, sna::betweenness)

## get the static network for every year
us.static <- get.networks(us.nd) # get each static network

## Various network and node level measures over time!
soyNodeDegree <- sapply(soyNets, sna::degree)
hist(soyNodeDegree) # most nodes in the network are not very connected!

USbtw <- sapply(us.static, sna::betweenness)
rownames(soyNodeBetweenness) = nd2%v%'id'
colnames(soyNodeBetweenness) = 1986:2013
